    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in islice(train_loader, batch_per_epoch):
                ...
                output = parallel_model(train_data, batch)
                if isinstance(output, tuple):
                    pred, aux = output
                    aux_loss = aux.get("aux_loss", None)
                else:
                    pred = output
                    aux_loss = None
                ...
                if util.get_rank() == 0 and batch_id % cfg.train.log_interval == 0:
                    ...
                    if aux_loss is not None:
                        logger.warning("auxiliary loss: %g" % aux_loss.item())
                    if isinstance(output, tuple) and "att_mean_per_branch" in aux:
                        att_dist = aux["att_mean_per_branch"]
                        if att_dist is not None:
                            logger.warning(f"attention distribution: {att_dist.cpu().numpy()}")
