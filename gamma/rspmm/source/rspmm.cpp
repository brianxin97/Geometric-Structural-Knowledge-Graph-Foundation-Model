...
void rspmm_forward_out_cpu(const int64_t *row_ptr, const int64_t *col_ind, const int64_t *layer_ind,
    ...
    parallel_for(0, num_row, 0, [&](int64_t row_start, int64_t row_end) {
        for (int64_t row = row_start; row < row_end; row++) {
            for (int64_t d = 0; d < dim; d++) output[row * dim + d] = NaryOp::zero;
            ...
            for (int64_t ptr = ptr_start; ptr < ptr_end; ptr++) {
                ...
                for (int64_t d = 0; d < dim; d++) {
                    scalar_t x = BinaryOp::forward(relation + layer * dim, input + col * dim, d, dim);
                    scalar_t y = w * x;
    ...
    parallel_for(0, num_row, 0, [&](int64_t row_start, int64_t row_end) {
        for (int64_t row = row_start; row < row_end; row++) {
            ...
            for (int64_t ptr = ptr_start; ptr < ptr_end; ptr++) {
                ...
                scalar_t w_grad = 0;
                for (int64_t d = 0; d < dim; d++) {
                    scalar_t x_d = BinaryOp::forward(relation + layer * dim, input + col * dim, d, dim);
                    scalar_t y_d = w * x_d;
                    scalar_t dout_dy = NaryOp::backward(output[row * dim + d], y_d);
                    scalar_t grad_z_d = output_grad[row * dim + d] * dout_dy * w;

                    scalar_t grad_z_d_other = 0;
                    if (BinaryOp::is_complex) {
                        int64_t half = dim / 2;
                        int64_t d_other = (d < half) ? d + half : d - half;
                        scalar_t x_other = BinaryOp::forward(relation + layer * dim, input + col * dim, d_other, dim);
                        scalar_t dout_dy_other = NaryOp::backward(output[row * dim + d_other], w * x_other);
                        grad_z_d_other = output_grad[row * dim + d_other] * dout_dy_other * w;
                    }

                    scalar_t dx_drel = BinaryOp::backward_lhs(relation + layer * dim, input + col * dim, grad_z_d, grad_z_d_other, d, dim);
                    scalar_t dx_din = BinaryOp::backward_rhs(relation + layer * dim, input + col * dim, grad_z_d, grad_z_d_other, d, dim);

                    w_grad += output_grad[row * dim + d] * dout_dy * x_d;
                    {
                        std::lock_guard<std::mutex> lock(relation_mutex[layer * dim + d]);
                        relation_grad[layer * dim + d] += dx_drel;
                    }
                    {
                        std::lock_guard<std::mutex> lock(input_mutex[col * dim + d]);
                        input_grad[col * dim + d] += dx_din;
                    }
                }
...
#define REGISTER_ALL_OPS(MUL_NAME, BINARY_OP) \
    DECLARE_FORWARD_IMPL(add, MUL_NAME, NaryAdd, BINARY_OP) \
    DECLARE_BACKWARD_IMPL(add, MUL_NAME, NaryAdd, BINARY_OP) \
    DECLARE_FORWARD_IMPL(min, MUL_NAME, NaryMin, BINARY_OP) \
    DECLARE_BACKWARD_IMPL(min, MUL_NAME, NaryMin, BINARY_OP) \
    DECLARE_FORWARD_IMPL(max, MUL_NAME, NaryMax, BINARY_OP) \
    DECLARE_BACKWARD_IMPL(max, MUL_NAME, NaryMax, BINARY_OP)

#define BIND_FORWARD_CPU(ADD, MUL) m.def("rspmm_" #ADD "_" #MUL "_forward_cpu", &at::rspmm_##ADD##_##MUL##_forward_cpu)
#define BIND_BACKWARD_CPU(ADD, MUL) m.def("rspmm_" #ADD "_" #MUL "_backward_cpu", &at::rspmm_##ADD##_##MUL##_backward_cpu)

#define BIND_ALL_CPU(MUL) \
    BIND_FORWARD_CPU(add, MUL); BIND_BACKWARD_CPU(add, MUL); \
    BIND_FORWARD_CPU(min, MUL); BIND_BACKWARD_CPU(min, MUL); \
    BIND_FORWARD_CPU(max, MUL); BIND_BACKWARD_CPU(max, MUL);

#ifdef CUDA_OP
#define BIND_FORWARD_CUDA(ADD, MUL) m.def("rspmm_" #ADD "_" #MUL "_forward_cuda", &at::rspmm_##ADD##_##MUL##_forward_cuda)
#define BIND_BACKWARD_CUDA(ADD, MUL) m.def("rspmm_" #ADD "_" #MUL "_backward_cuda", &at::rspmm_##ADD##_##MUL##_backward_cuda)

#define BIND_ALL_CUDA(MUL) \
    BIND_FORWARD_CUDA(add, MUL); BIND_BACKWARD_CUDA(add, MUL); \
    BIND_FORWARD_CUDA(min, MUL); BIND_BACKWARD_CUDA(min, MUL); \
    BIND_FORWARD_CUDA(max, MUL); BIND_BACKWARD_CUDA(max, MUL);
#endif
...
REGISTER_ALL_OPS(complex, BinaryComplex)
REGISTER_ALL_OPS(split_complex, BinarySplitComplex)
REGISTER_ALL_OPS(dual, BinaryDual)
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    BIND_ALL_CPU(complex)
    BIND_ALL_CPU(split_complex)
    BIND_ALL_CPU(dual)
    ...
    BIND_ALL_CUDA(complex)
    BIND_ALL_CUDA(split_complex)
    BIND_ALL_CUDA(dual)
#endif
}
